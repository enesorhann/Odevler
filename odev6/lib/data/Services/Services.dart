...
import 'package:odev6/data/database/DatabaseHelper.dart';
...
class Services {
  ...
  Future<void> guncelle(
      int userID, String userName, String password, String? imagePath) async {
    var db = await DatabaseHelper().veritabaniErisim();
    var info = Map<String, dynamic>();
    info["userName"] = userName;
    info["password"] = password;
  ...
  Future<void> insertOrUpdateGelirGider(int userID, String tarih, double gelir,
      double gider, String gelirAciklama, String giderAciklama,
      {int? harcamaID}) async {
    try {
      var db = await DatabaseHelper().veritabaniErisim();
  ...
  Future<List<Harcamalar>> getGelirGiderByUserID(int userID) async {
    var db = await DatabaseHelper().veritabaniErisim();
    var maps =
        await db.query('gelir_gider', where: 'userID = ?', whereArgs: [userID]);
